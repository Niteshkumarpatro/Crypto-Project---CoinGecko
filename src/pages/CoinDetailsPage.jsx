...
function CoinDetailsPage() {
    ...
    const { isError, isLoading, data: coin } = useQuery({
        queryKey: ['coin-details', coinId], queryFn: () => fetchCoinDetails(coinId),
        cacheTime: 1000 * 60 * 2,
        staleTime: 1000 * 60 * 2
    });
    ...
    return (
        <div className="flex flex-col md:flex-row">
            <div className="md:w-1/3 w-full flex flex-col items-center mt-6 md:mt-0 border-r-2 border-gray-300">
                <img
                    alt={coin?.name}
                ...
                <div className="w-full flex flex-col md:flex-row md:justify-around">
                    <div className="flex items-center mb-4 md:mb-0">
                        ...
                        <span className="ml-2 text-xl">{coin?.market_cap_rank}</span>
                    </div>
                    <div className="flex items-center mb-4 md:mb-0">
                        <h2 className="text-xl font-bold text-yellow-400">Current Price:</h2>
                        <span className="ml-2 text-xl">{coin?.market_data.current_price[currency]}</span>
                    </div>
                </div>
            </div>
